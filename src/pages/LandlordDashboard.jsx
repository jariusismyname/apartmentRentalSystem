...
export default function LandlordDashboard({ applications, updateStatus, properties, setProperties }) {
  ...
  // 3. Save modified fields handler
  const handleSaveEdit = (e) => {
    ...
    setEditingPropertyId(null); // Close the editing menu box
  };
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.tabBar}>
        ...
        >
          🏢 Manage My Listings ({properties.length})
        </button>
      </div>

      {/* --- TAB CONTENT 1: APPLICATIONS LIST --- */}
              ...
              <tbody>
                {applications.map(app => (
                  <tr key={app.id} style={styles.tableRow}>
                    <td style={styles.td}><strong>{app.fullName}</strong></td>
      ...
      {/* --- TAB CONTENT 2: MANAGE PROPERTIES INVENTORY --- */}
      {activeTab === 'properties' && (
        <div>
          {/* Modal Overlay / Inline Form for handling updates */}
          {editingPropertyId && (
            <div style={styles.editOverlay}>
              <form onSubmit={handleSaveEdit} style={styles.editForm}>
                ...
                <div style={{ display: 'flex', gap: '12px' }}>
                  <div style={{ flex: 1 }}>
                    <label style={styles.formLabel}>Monthly Rent ($)</label>
                    <input 
                      type="number" 
                ...
                <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', marginTop: '20px' }}>
                  <button type="button" onClick={() => setEditingPropertyId(null)} style={styles.cancelBtn}>Discard</button>
          ...
          {/* Core Properties Directory List */}
...
const styles = {
  ...
  // Real Estate Management Tab Elements
